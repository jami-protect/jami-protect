...
(function(){
  ...
  const indexLabel=document.querySelector("[data-game-index]");
  const prev=document.querySelector("[data-game-prev]");
  const next=document.querySelector("[data-game-next]");
  ...
  function render(){
    cards.forEach((card,i)=>{
      ...
      else if(delta===1)card.classList.add("near");
    });
    const card=cards[active];
    if(title)title.textContent=card.dataset.name||"";
    if(note)note.textContent=card.dataset.note||"Research knowledge";
    if(indexLabel)indexLabel.textContent=String(active+1).padStart(2,"0")+" / "+String(cards.length).padStart(2,"0");

    // center the active card without letting the row hang outside the section
    const carousel=document.querySelector(".game-carousel");
    if(carousel && card){
      const c=carousel.getBoundingClientRect(), r=card.getBoundingClientRect();
      const shift=(c.left+c.width/2)-(r.left+r.width/2);
      carousel.style.transform=`translateX(${shift}px)`;
      carousel.style.transition="transform .45s cubic-bezier(.2,.75,.2,1)";
    }
  }
  ...
  cards.forEach((card,i)=>card.addEventListener("click",()=>{active=i;render();restart()}));
