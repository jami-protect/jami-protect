...
document.querySelectorAll('[data-language-dropdown] a').forEach(a=>a.addEventListener('click',()=>a.closest('details')?.removeAttribute('open')));
document.addEventListener('click',e=>document.querySelectorAll('details[data-language-dropdown][open]').forEach(d=>{if(!d.contains(e.target))d.removeAttribute('open')}));


// V6.9.2 language dropdown hardening: one menu at a time + Escape support.
(() => {
  const menus = [...document.querySelectorAll('details[data-language-dropdown]')];
  menus.forEach(menu => {
